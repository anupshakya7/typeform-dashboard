...
class AnswerController extends Controller
{
    ...
    {
        $form = Form::where('form_id', $request->survey)->first();
        ...
        if ($request->filled('country')) {
            if ($form->form_type == 1) {
                $surveyQuery->where('country', $request->country);
            }
            // if only required
            // else{
        ...
        if ($request->filled('state')) {
            if ($form->form_type == 1) {
                $surveyQuery->where('state', $request->state);
            }
        }
        ...
        foreach ($survey as $row) {
            ...
            if (optional($row->form)->extraQuestions !== null) {
    ...
    {
        $form = Form::with('extraQuestions')->where('form_id', $request->survey)->first();
        ...
        if (count($form->extraQuestions) > 0) {
            ...
            return response()->json([
                'error' => 'There is no any extra Questions',
            ]);
        }
    }
    ...
    public function globalCountry($country)
    {
        $country = NCountry::where('code', $country)->pluck('name')->first();
